...
var Radbibliotek = require('./Radbibliotek');
...
Messages.prototype.saveMessage = function (message) {
    _.forEach(['ISK','AVS','MOT','EKV'], function (key) {
        var isFound = !!_.find(this.get(key), function (msg) {
...
Messages.prototype._initMessageFormat = function (format) {
    format.getLines = function () {
        var lineNumbers = format.lines || [].concat(format.header || formats.header, format.body || []);
        return _.map(lineNumbers, function (number) {
            return Radbibliotek[number];
        });
    };
};
