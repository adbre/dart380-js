...
Message.prototype.backspace = function (row, col) {
    var prev = this.nextWriteableCell(row, col, -1);

    var last = this.nextWriteableCell(row, col, 1, function (ch) {
        return ch !== ' ';
    });
    if (this._template[last.row][col] !== ' ') {
        last = this.nextWriteableCell(last.row, last.col, -1);
    }

    var length;
    if (last.row - prev.row !== 0) {
        length = (16 - prev.col) + (last.col) + ((last.row - prev.row - 1) * 16);
    } else {
        length = last.col - prev.col;
    }

    var text = this.read(prev.row, prev.col, length);
    text = text.substr(1, text.length - 1) + ' ';
    this.writeAt(prev.row, prev.col, text);
    return prev;
};

Message.prototype.delete = function (row, col) {

    var prev = { row: row, col: col };
    ...
    var text = this.read(row, col, length);
    text = text.substr(1, text.length - 1) + ' ';
    this.writeAt(row, col, text);
    return prev;
};

Message.prototype.nextWriteableCell = function (row, col, step, stopFn) {
    if (_.isUndefined(stopFn)) {
        stopFn = function (ch) {
            return ch === ' ';
        };
    }
    ...
    } while (!stopFn(this._template[row][col]));
...
Message.prototype.setTimestamp = function (time) {
    if (!_.isString(time)) {
        time = sprintf('%02d%02d%02d', time.day, time.hour, time.minute);
    }
